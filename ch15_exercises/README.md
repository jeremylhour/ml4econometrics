...
|-----------------------------------|----------------------------|------------------------------|
| University of Oxford & Nuffield College |                            | Capital Fund Management       |
| CREST                             |                            | CREST                        |

**Version:** *May 17, 2025*

```# Regression as a Weighting Estimator
...
1. 
   ...
   2. The propensity score \(p(X)\) can be estimated by logistic regression, which yields an estimator for \(\beta_0\). Then, \(\hat \tau\) can be estimated by:
      \[
...
2. 
   ...
   2. The most efficient method is maximum likelihood estimation, given by the equation (LOGIT) in section (2.3.2) of the book, where \(Y_i\) is replaced by \(D_i\). The estimator \(\hat \tau\) of \(\tau_0\) found in question 1.a, using the maximum likelihood estimator \(\hat \beta\), will be asymptotically normal. This is straightforward to show.
   
   3. In high-dimensional settings, it will be necessary to use machine learning methods whereby the estimator of \(\beta_0\) will not be asymptotically normal (for example, a Lasso logistic regression as mentioned in the prompt). This will introduce bias in \(\hat \tau\) since generally:
      \[
      \mathbb{E}[\partial_\beta m(W, \tau_0, \beta_0)] = - \mathbb{E}[X D Y_0] \neq 0.
      \]
...
3. 
   1. The Conditional Independence Assumption (CIA) implies \(Y_0 - X' \gamma_0 \perp D \mid X\), hence
      \[
      \mathbb{E}[D X (Y_0 - X' \gamma_0)] = \mathbb{E}[X \mathbb{E}[\varepsilon \mid X] \mathbb{E}[D \mid X]] = 0.
      \]
   
   2. Drawing inspiration from Chapter 5, we propose:
      \[
...
4. One can rely on the estimators proposed in section 5.3 and use Theorem 5.1.# Voting Model
...
5. In the corresponding chapter, the asymptotic normality of \(\widehat{\tau}\) is proven for the affine-quadratic model, which requires that:
   1. either \( g \) is an affine function. In this case, we return to the case seen in the corresponding chapter.
   2. or \( g \) is a quadratic function, which is allowed by the theorem but requires using an estimator for \(\beta_0\) from a nonlinear index model in the first stage.# Heterogeneity of the Male-Female Wage Gap
...
2.  
   1. \(X_i\) can contain many variables: hours worked, experience, experience squared, age, type of education, years of education, geographic location, nationality, marital status, number of young children, total number of children, industry, psychological traits such as conscientiousness and openness, etc.
   2. In this case, a simple OLS estimator works thanks to the exogeneity assumption.
   3. No, this is not the case. Given the sparse structure, one must use the double selection procedure seen in class, employing a Lasso in the first two steps — a brief description of the procedure is necessary here.
   4. \[
...
3.  
   1. This is true. Indeed, in this case,
      \[
   ...
   2. Using the notation \(\theta = (\theta_k)_{k=1, \ldots, K}\), we have:
      \[
   ...
   3. An orthogonal (or "immunized") moment function \(\psi\) for \((\theta_1, \ldots, \theta_K)\) takes the form
      ...
      1. The first \(K\) steps consist of regressing each element of \(Z_i\) on \(X_i\) for the subsample of women using a Lasso,
      2. The \((K+1)^\text{th}\) step is a Lasso regression of \(\ln W_i\) on \(X_i\),
      3. The final step is a regression of \(\ln W_i\) on \(F_i Z_i\) and the union of all elements of \(X_i\) previously selected.
...
- (Bonus)# Drought and incentives to save water {#exo:imai}
...
- We replace \(\mathbb{E}[D]\) by \(Z' \gamma\) and add a penalty as well as a potential sparsity assumption to handle the potential high dimensionality of \(\gamma\), which is a nuisance parameter.
  ...
  \end{aligned}
  \]# Synthetic Control and Regularization
...
2. 
   1. This is a least squares program.  
   ...
   2. \(\hat \omega_i\) is the weight given to untreated unit \(i\) in reproducing the treated unit. It corresponds to the partial correlation of the time series of that particular unit with that of the treated unit. It can be negative if they are negatively correlated.

   3. Several: it cannot always be calculated, it allows for extrapolation.
...
4. For this question, we add to (OBJ) the three constraints: \(\omega_i \geq 0\) for \(i=2, \ldots, N+1\), \(\sum_{i=2}^{N+1} \omega_i = 1\), and \(\mu = 0\).
   ...
   1. This is the synthetic control estimator.

   2. No, it is generally not unique.

5. 
   1. This helps to regularize the problem.

   2. \(\alpha = 0\) corresponds to the Ridge estimator:
      \[
   ...
   3. Some form of cross-validation using either the temporal dimension or the cross-sectional dimension among the untreated units.